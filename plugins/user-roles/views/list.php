				...
				<tr>
					...
					<th>
						<?php if (user_can('add_user')) : ?>
							<a href="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/add" class="btn btn-bd-primary btn-sm">
						...
						<tr>
									...
									<?php if (!empty($perms)) : $num = 0 ?>
										<?php foreach ($perms as $perm) : $num++ ?>
											<div class="form-check col-md-6">
												<input <?= in_array($perm, $row->permissions ?? []) ? ' checked ' : '' ?> name="checkbox_<?= $row->id ?>_<?= $num ?>" class="form-check-input" type="checkbox" value="<?= $perm ?>" id="check<?= $num ?>">
							...
							<td>
								...
								<?php if (user_can('edit_role')) : ?>
									<a href="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/edit/<?= $row->id ?>" class="btn btn-warning btn-sm">
										<i class="fa-solid fa-pen-to-square"></i> Edit
									</a>
								<?php endif ?>

								<?php if (user_can('delete_role')) : ?>
									<a href="<?= ROOT ?>/<?= $admin_route ?>/<?= $plugin_route ?>/delete/<?= $row->id ?>" class="btn btn-danger btn-sm">
