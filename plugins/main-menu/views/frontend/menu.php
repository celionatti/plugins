			...
			<?php if (!empty($links)) : ?>
				<?php foreach ($links as $link) : ?>
					...
					<?php if (user_can($link->permission)) : ?>
						<li><a class="<?= !empty($link->children) ? 'desktop-item' : '' ?>" href="<?= ROOT ?>/<?= $link->slug ?>"><?= esc($link->title) ?></a>

							<?php if (!empty($link->children) && empty($link->is_mega)) : ?>

								<input type="checkbox" id="showDrop">
								<label for="showDrop" class="mobile-item"><?= esc($link->title) ?></label>
								<ul class="drop-menu">
							...
							<?php elseif (!empty($link->children) && !empty($link->is_mega)) : ?>
								...
								<label for="showMega" class="mobile-item"><?= esc($link->title) ?></label>
								<div class="mega-box">
									<div class="content">
										<div class="row">
											<?php if (!empty($row->mega_image) && file_exists($row->mega_image)) : ?>
												<img src="<?= getThumbnail($row->mega_image, 246, 164) ?>" alt="">
											<?php endif ?>
										</div>
